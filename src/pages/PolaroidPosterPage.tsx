...
const GOOGLE_FONTS = [
  "Dancing Script", "Caveat", "Pacifico", "Satisfy", "Sacramento",
  "Montserrat", "Inter", "Space Mono", "Playfair Display", "Lora"
];
...
const PRESETS = [
  {
    id: 'sweet-trio',
    label: 'Sweet Trio',
    desc: 'Three layered, slightly tilted vintage Polaroid frames with handwritten notes.',
    texts: {
      caption1: 'Summer Sunset',
      caption2: 'Where we laughed',
      caption3: 'Forever Us',
      footer: 'OUR UNFORGETTABLE JOURNEY'
    },
    styling: {
      bg: '#fafaf9',
      frameBg: '#ffffff',
  ...
  },
  {
    ...
    desc: 'Two perfectly aligned, clean structural frames for a modern gallery look.',
    texts: {
      caption1: 'The Beginning',
      caption2: 'The Present',
      caption3: '',
      footer: 'THEN & NOW — EST. 2024'
    },
    styling: {
      bg: '#18181b',
      frameBg: '#f4f4f5',
...
export default function PolaroidPosterPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [activePreset, setActivePreset] = useState<string>('sweet-trio');
  ...
  const [frames, setFrames] = useState<PolaroidFrameState[]>([
    { imgUrl: null, caption: 'SUMMER SUNSET' },
    { imgUrl: null, caption: 'WHERE WE LAUGHED' },
    { imgUrl: null, caption: 'FOREVER US' }
  ]);
  const [footerText, setFooterText] = useState('OUR UNFORGETTABLE JOURNEY');
  ...
  const [layoutStyle, setLayoutStyle] = useState<string>('tilted');

  const showToast = (msg: string) => {
    const el = document.querySelector('.pol-toast') as HTMLElement;
    if (el) {
      el.innerText = msg;
      el.style.opacity = '1';
  ...
  const applyPreset = (presetId: string) => {
    ...
    setFrames([
      { imgUrl: frames[0].imgUrl, caption: preset.texts.caption1 },
      { imgUrl: frames[1].imgUrl, caption: preset.texts.caption2 },
      { imgUrl: frames[2].imgUrl, caption: preset.texts.caption3 }
    ]);
    setFooterText(preset.texts.footer);

    setBgColor(preset.styling.bg);
    setFrameBgColor(preset.styling.frameBg);
    setTextColor(preset.styling.text);
    setFontFamily(preset.styling.font);
    setLayoutStyle(preset.styling.layout);
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      if (isAdmin) {
        setIsLocked(false);
        if (!token || token === 'demo-token' || token === 'polaroid' || token.length < 10) {
      ...
      try {
        ...
        if (data.design_state) {
          ...
          setFrameBgColor(ds.frameBgColor || '#ffffff');
          setTextColor(ds.textColor || '#1c1917');
          setFontFamily(ds.fontFamily || 'Caveat');
          setLayoutStyle(ds.layoutStyle || 'tilted');
        }
      } catch (err) {
        if (!isAdmin) {
  ...
  const createPolaroidGroup = async (
    cw: number, ch: number,
    fX: number, fY: number, fW: number, fAngle: number,
    imgUrl: string | null, captionText: string
    ...
    const picPadding = fW * 0.07;
    const picW = fW - picPadding * 2;
    const picH = picW;

    const picPlaceholder = new fabric.Rect({
      left: picPadding,
      top: picPadding,
      width: picW,
      height: picH,
    ...
    const caption = new fabric.Text(captionText.toUpperCase(), {
      left: fW / 2,
      top: picPadding + picH + (fH - (picPadding + picH)) / 2,
      originX: 'center',
      originY: 'center',
      fontSize: fW * 0.08,
    ...
    const pGroup = new fabric.Group([card, picPlaceholder, caption], {
      left: fX,
      top: fY,
      originX: 'center',
      originY: 'center',
  ...
  const drawPolaroidStructure = async () => {
    ...
    if (layoutStyle === 'tilted') {
      const fW = cw * 0.38;

      const p1 = await createPolaroidGroup(cw, ch, cx - cw * 0.18, cy - 40, fW, -8, frames[0].imgUrl, frames[0].caption);
      const p2 = await createPolaroidGroup(cw, ch, cx + cw * 0.18, cy - 25, fW, 6, frames[1].imgUrl, frames[1].caption);
      const p3 = await createPolaroidGroup(cw, ch, cx, cy + 90, fW, -2, frames[2].imgUrl, frames[2].caption);

      canvas.add(p1, p2, p3);
    } else {
      const fW = cw * 0.36;

      const p1 = await createPolaroidGroup(cw, ch, cx - cw * 0.20, cy, fW, 0, frames[0].imgUrl, frames[0].caption);
      const p2 = await createPolaroidGroup(cw, ch, cx + cw * 0.20, cy, fW, 0, frames[1].imgUrl, frames[1].caption);
  ...
  useEffect(() => {
    ...
    isCheckingToken, tokenError, frames, footerText, bgColor, frameBgColor, textColor, fontFamily, layoutStyle
  ]);
  ...
  const handlePhotoUpload = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setFrames(prev => {
        const next = [...prev];
        next[index] = { ...next[index], imgUrl: url };
        return next;
      });
      showToast(`Photo ${index + 1} uploaded.`);
    };
  ...
  const removePhoto = (index: number) => {
    setFrames(prev => {
      const next = [...prev];
      next[index] = { ...next[index], imgUrl: null };
      return next;
    });
    showToast(`Photo ${index + 1} removed.`);
  };
    ...
    setTimeout(async () => {
      try {
        ...
        if (format === 'png') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `polaroid-poster.svg`;
          a.click();
          URL.revokeObjectURL(url);
        }

        const designStateJSON = {
          canvasSize, orientation, frames, footerText, bgColor, frameBgColor, textColor, fontFamily, layoutStyle
        };
  ...
  return (
    <div className={`soundwave-poster-page ${isLocked ? 'locked-mode' : ''}`}>
      ...
      <div id="panel" className={isLocked ? 'hidden' : ''}>
        ...
        <button className={`accordion-btn${openSections.photos ? ' open' : ''}`} onClick={() => toggleAccordion('photos')}>
          &#128247; Upload Photos<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.photos ? ' open' : ''}`}>
          {frames.slice(0, layoutStyle === 'tilted' ? 3 : 2).map((frame, idx) => (
            <div className="form-row" key={`photo-ctrl-${idx}`} style={{ borderBottom: '1px solid var(--panel-border)', paddingBottom: '12px', marginBottom: '12px' }}>
              <label style={{ color: 'var(--accent)', fontWeight: 'bold' }}>Polaroid Frame {idx + 1}</label>
              <div className="flex gap-2 items-center mt-2">
                <button className="upload-compact-btn" onClick={() => document.getElementById(`pol-upload-${idx}`)?.click()}>
                  <Upload className="w-3.5 h-3.5" /> Upload
                </button>
                <input type="file" id={`pol-upload-${idx}`} accept="image/*" onChange={(e) => handlePhotoUpload(idx, e)} style={{ display: 'none' }} />
                
                {frame.imgUrl && (
                  <button className="trash-btn" onClick={() => removePhoto(idx)} title="Remove Photo">
              ...
              <div className="mt-3">
                ...
                <input type="text" value={frame.caption} onChange={(e) => updateCaption(idx, e.target.value)} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`accordion-content${openSections.styling ? ' open' : ''}`}>
          <div className="form-row">
            ...
            <select value={layoutStyle} onChange={(e) => setLayoutStyle(e.target.value)}>
              <option value="tilted">Sweet Trio (3 Frames Tilted)</option>
              <option value="duo-clean">Minimalist Duo (2 Frames Straight)</option>
            </select>
          </div>
          <div className="form-row">
            <label>Handwritten Font</label>
          ...
          <div className="form-row">
            <label>Polaroid Card Color</label>
            <div className="color-row">
              <input type="color" value={frameBgColor} onChange={(e) => setFrameBgColor(e.target.value)} />
              <input type="text" value={frameBgColor} onChange={(e) => setFrameBgColor(e.target.value)} />
            </div>
          </div>
          <div className="form-row">
            ...
            <div className="review-warning-box">
              ...
              <div>
                ...
                <p className="text-red-200/80 text-sm leading-relaxed">
                  Please review your polaroid gallery carefully. Check photo scales, spellings, and tilted angles.
                </p>
              </div>
            </div>
