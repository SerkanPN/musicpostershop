...
const GOOGLE_FONTS = [
  "Inter", "Montserrat", "Oswald", "Bebas Neue", "Space Mono", "Syncopate",
  "Playfair Display", "Prata", "Vollkorn", "Lora", "Crimson Text", "Cinzel"
];
...
const PRESETS = [
  {
    id: 'stoic-wisdom',
    label: 'Stoic Wisdom',
    desc: 'Bold, structured quote by Marcus Aurelius. Classical stoic look.',
    texts: {
      header: 'MARCUS AURELIUS',
      meta: 'MEDITATIONS — BOOK IV',
      title: 'THE IMPEDIMENT TO ACTION ADVANCES ACTION.',
      body: 'WHAT STANDS IN THE WAY BECOMES THE WAY. YOU HAVE POWER OVER YOUR MIND - NOT OUTSIDE EVENTS. REALIZE THIS, AND YOU WILL FIND STRENGTH.',
      footer: 'STOIC ACADEMY OF ATHENS'
    },
    styling: {
      bg: '#fcfbf7',
      text: '#1a1a1a',
      ...
      align: 'left',
      gridStyle: 'swiss-borders'
    }
  },
  {
    id: 'manifesto-grid',
    label: 'Personal Manifesto',
    desc: 'Swiss-style numbered list layout for personal manifestos and rules.',
    texts: {
      header: 'PERSONAL RULES',
      meta: 'DAILY MANIFESTO v1.2',
      title: 'DISCIPLINE IS FREEDOM.',
      body: '01. WAKE UP AT DAWN.\n02. COLD PLUNGE & FOCUS.\n03. DO NOT SEEK COMFORT.\n04. EMBRACE THE GRIND.\n05. STAY SILENT, WORK HARDER.',
      footer: 'RULE YOURSELF FIRST'
    },
    styling: {
      bg: '#09090b',
      text: '#f4f4f5',
      ...
      font: 'Space Mono',
      align: 'left',
      gridStyle: 'modern-clean'
    }
  },
  {
    id: 'cinema-monologue',
    label: 'Cinema Monologue',
    desc: 'High-contrast, dramatic layout for legendary movie quotes.',
    texts: {
      header: 'TYLER DURDEN',
      meta: 'FIGHT CLUB (1999)',
      title: 'IT IS ONLY AFTER WE HAVE LOST EVERYTHING...',
      body: '...THAT WE ARE FREE TO DO ANYTHING. THIS IS YOUR LIFE AND IT IS ENDING ONE MINUTE AT A TIME. DO NOT LET SECONDS SLIP AWAY.',
      footer: 'PAPERCUT PRODUCTIONS'
    },
    styling: {
      bg: '#171717',
      text: '#fafafa',
      ...
      font: 'Montserrat',
      align: 'left',
      gridStyle: 'bold-asymmetric'
    }
  },
  {
    id: 'corporate-mission',
    label: 'Corporate Values',
    desc: 'Strict Swiss-grid architecture for boardroom and company mission statements.',
    texts: {
      header: 'CORE VALUES',
      meta: 'MISSION & VISION',
      title: 'INNOVATE. DISRUPT. DELIVER.',
      body: 'WE BUILD SYSTEMS THAT OUTLAST GENERATIONS. WE FOCUS ON UNCOMPROMISING QUALITY, EXTREME COOPERATIVE TRANSPARENCY, AND SYSTEMATIC SCALE.',
      footer: 'GLOBAL CORP LABS'
    },
    styling: {
      bg: '#fafafa',
      text: '#0f172a',
...
export default function TypographyPosterPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [activePreset, setActivePreset] = useState<string>('stoic-wisdom');
  ...
  const [headerText, setHeaderText] = useState('MARCUS AURELIUS');
  const [metaText, setMetaText] = useState('MEDITATIONS — BOOK IV');
  const [titleText, setTitleText] = useState('THE IMPEDIMENT TO ACTION ADVANCES ACTION.');
  const [bodyText, setBodyText] = useState('WHAT STANDS IN THE WAY BECOMES THE WAY. YOU HAVE POWER OVER YOUR MIND - NOT OUTSIDE EVENTS. REALIZE THIS, AND YOU WILL FIND STRENGTH.');
  const [footerText, setFooterText] = useState('STOIC ACADEMY OF ATHENS');
  ...
  const [gridStyle, setGridStyle] = useState<string>('swiss-borders');

  const showToast = (msg: string) => {
    const el = document.querySelector('.typ-toast') as HTMLElement;
    if (el) {
      el.innerText = msg;
      el.style.opacity = '1';
  ...
  const applyPreset = (presetId: string) => {
    ...
    setHeaderText(preset.texts.header);
    setMetaText(preset.texts.meta);
    setTitleText(preset.texts.title);
    setBodyText(preset.texts.body);
    ...
    setTextAlign(preset.styling.align as any);
    setGridStyle(preset.styling.gridStyle);
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      if (isAdmin) {
        setIsLocked(false);
        if (!token || token === 'demo-token' || token === 'typography' || token.length < 10) {
      ...
      try {
        ...
        if (data.design_state) {
          ...
          setHeaderText(ds.headerText || '');
          setMetaText(ds.metaText || '');
          setTitleText(ds.titleText || '');
          setBodyText(ds.bodyText || '');
          setFooterText(ds.footerText || '');
          setBgColor(ds.bgColor || '#fcfbf7');
          setTextColor(ds.textColor || '#1a1a1a');
          setAccentColor(ds.accentColor || '#9a3412');
          setFontFamily(ds.fontFamily || 'Playfair Display');
          setTextAlign(ds.textAlign || 'left');
          setGridStyle(ds.gridStyle || 'swiss-borders');
        }
      } catch (err) {
        if (!isAdmin) {
  ...
  const drawTypographyStructure = () => {
    ...
    const drawGridLines = () => {
      if (gridStyle === 'swiss-borders') {
        ...
        const sepLine = new fabric.Line([marginL, ch * 0.16, marginR, ch * 0.16], {
          stroke: textColor,
          strokeWidth: 1.5,
          selectable: false
        });
        ...
        const titleSepLine = new fabric.Line([marginL, ch * 0.44, marginR, ch * 0.44], {
          stroke: textColor,
          strokeWidth: 1.5,
          selectable: false
        });
        canvas.add(titleSepLine);

        const footerSepLine = new fabric.Line([marginL, ch * 0.84, marginR, ch * 0.84], {
          stroke: textColor,
          strokeWidth: 1.5,
          selectable: false
        });
        canvas.add(footerSepLine);
      } else if (gridStyle === 'bold-asymmetric') {
        const structuralBlock = new fabric.Rect({
          left: marginL,
          top: ch * 0.18,
          width: cw * 0.03,
          height: ch * 0.60,
        ...
        canvas.add(structuralBlock);
      }
    };

    drawGridLines();

    const tHeader = new fabric.Text(headerText.toUpperCase(), {
      left: textAlign === 'center' ? cw / 2 : textAlign === 'left' ? marginL + (gridStyle === 'swiss-borders' ? 15 : 0) : marginR - (gridStyle === 'swiss-borders' ? 15 : 0),
      top: ch * 0.11,
      originX: textAlign,
      fontSize: 14,
    ...
    const tMeta = new fabric.Text(metaText.toUpperCase(), {
      left: textAlign === 'center' ? cw / 2 : textAlign === 'left' ? marginL + (gridStyle === 'swiss-borders' ? 15 : 0) : marginR - (gridStyle === 'swiss-borders' ? 15 : 0),
      top: ch * 0.13,
      originX: textAlign,
      fontSize: 10,
    ...
    const titleIndent = (gridStyle === 'bold-asymmetric') ? cw * 0.08 : (gridStyle === 'swiss-borders' ? 15 : 0);
    const tTitle = new fabric.Textbox(titleText.toUpperCase(), {
      left: textAlign === 'center' ? cw / 2 : textAlign === 'left' ? marginL + titleIndent : marginR - titleIndent,
      top: ch * 0.22,
      width: innerW - titleIndent,
      originX: textAlign,
      textAlign: textAlign,
      fontSize: 32,
      fontFamily: fontFamily,
    ...
    const bodyY = (gridStyle === 'swiss-borders') ? ch * 0.48 : ch * 0.45;
  ...
  useEffect(() => {
    ...
    isCheckingToken, tokenError, headerText, metaText, titleText, bodyText, 
    footerText, bgColor, textColor, accentColor, fontFamily, textAlign, gridStyle
  ]);
    ...
    setTimeout(async () => {
      try {
        ...
        if (format === 'png') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `typography-poster.svg`;
          a.click();
          URL.revokeObjectURL(url);
        }

        const designStateJSON = {
          canvasSize, orientation, headerText, metaText, titleText, bodyText,
          footerText, bgColor, textColor, accentColor, fontFamily, textAlign, gridStyle
        };
    ...
    <div className={`soundwave-poster-page ${isLocked ? 'locked-mode' : ''}`}>
      ...
      <div id="panel" className={isLocked ? 'hidden' : ''}>
        ...
        <div className={`accordion-content${openSections.textFields ? ' open' : ''}`}>
          <div className="form-row">
            <label>Top Left Header</label>
            <input type="text" value={headerText} onChange={(e) => setHeaderText(e.target.value)} />
          </div>
          <div className="form-row">
            <label>Top Right Sub-Header</label>
            <input type="text" value={metaText} onChange={(e) => setMetaText(e.target.value)} />
          </div>
          <div className="form-row">
            <label>Big Hook Title</label>
            <input type="text" value={titleText} onChange={(e) => setTitleText(e.target.value)} />
          </div>
          <div className="form-row">
            <label>Main Body Paragraph</label>
            <textarea 
              value={bodyText} 
              ...
              style={{
                ...
                fontFamily: 'inherit', minHeight: '100px', resize: 'vertical', outline: 'none'
              }}
            />
          </div>
          <div className="form-row">
        ...
        <button className={`accordion-btn${openSections.styling ? ' open' : ''}`} onClick={() => toggleAccordion('styling')}>
          &#127912; Swiss Layout Styles<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.styling ? ' open' : ''}`}>
          <div className="form-row">
            <label>Swiss Grid Structure</label>
            <select value={gridStyle} onChange={(e) => setGridStyle(e.target.value)}>
              <option value="swiss-borders">Classic Swiss Borders</option>
              <option value="bold-asymmetric">Bold Asymmetric Stripe</option>
              <option value="modern-clean">Minimal Grid</option>
            </select>
          </div>
          <div className="form-row">
            ...
            <div className="review-warning-box">
              ...
              <div>
                ...
                <p className="text-red-200/80 text-sm leading-relaxed">
                  Please review your manifesto carefully. Check spelling, structure, and paragraph grid.
                </p>
              </div>
            </div>
