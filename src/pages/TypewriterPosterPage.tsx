...
const GOOGLE_FONTS = [
  "Courier Prime", "Special Elite", "Space Mono", "Inconsolata", "VT323",
  "Playfair Display", "Lora", "Crimson Text", "Prata", "Montserrat"
];
...
const PRESETS = [
  {
    id: 'love-letter',
    label: 'Love Letter',
    desc: 'Nostalgic typed vows or a love letter on vintage parchment paper.',
    texts: {
      date: 'OCTOBER 24, 1989',
      salutation: 'DEAREST LILY,',
      body: 'I AM WRITING THIS ON THE OLD UNDERWOOD TYPEWRITER WE FOUND AT THE FLEA MARKET. EVERY CLICK OF THE KEYS REMINDS ME OF THE CONSTANT BEAT OF MY HEART FOR YOU.\n\nIF I HAD TO CHOOSE BETWEEN BREATHING AND LOVING YOU, I WOULD USE MY LAST BREATH TO TELL YOU THAT I LOVE YOU FOREVER.',
  ...
  },
  {
    id: 'poetry-recital',
    label: 'Poem Recital',
    desc: 'An elegant literary layout for classic poems or personal verses.',
    texts: {
      date: 'ROME, 1964',
      salutation: 'SONNET XVII',
      body: 'I LOVE YOU WITHOUT KNOWING HOW, OR WHEN, OR FROM WHERE.\nI LOVE YOU DIRECTLY WITHOUT PROBLEMS OR PRIDE:\nI LOVE YOU LIKE THIS BECAUSE I DON’T KNOW ANY OTHER WAY TO LOVE,\n\nEXCEPT IN THIS FORM IN WHICH I AM NOT NOR ARE YOU,\nSO CLOSE THAT YOUR HAND UPON MY CHEST IS MINE,\nSO CLOSE THAT YOUR EYES CLOSE WITH MY DREAMS.',
      signoff: 'PABLO NERUDA',
      signature: 'SELECTED VERSES'
    },
    styling: {
      bg: '#f4edd8', 
      ink: '#1c1917', 
  ...
  },
  {
    ...
    desc: 'Raw, honest words typed with direct carbon ink on typewriter layout.',
    texts: {
      date: 'MIDNIGHT ESSAY',
      salutation: 'TO MY PARTNER,',
      body: 'SOMETIMES THE WORDS GET TANGLED BEFORE THEY LEAVE MY MOUTH. I KNOW I HAVE BEEN DISTANT, AND I AM DEEPLY SORRY FOR THE HEAVINESS THAT CAUSED.\n\nI PROMISE TO BE BETTER, TO LISTEN DEEPER, AND TO HOLD YOU TIGHTER WHEN THE WORLD GETS TOO LOUD.',
      signoff: 'WITH ALL MY REMORSE,',
      signature: 'JONATHAN'
    },
    styling: {
      bg: '#ffffff', 
...
export default function TypewriterPosterPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [activePreset, setActivePreset] = useState<string>('love-letter');
  ...
  const [dateText, setDateText] = useState('OCTOBER 24, 1989');
  const [salutationText, setSalutationText] = useState('DEAREST LILY,');
  const [bodyText, setBodyText] = useState('I AM WRITING THIS ON THE OLD UNDERWOOD TYPEWRITER WE FOUND AT THE FLEA MARKET. EVERY CLICK OF THE KEYS REMINDS ME OF THE CONSTANT BEAT OF MY HEART FOR YOU.\n\nIF I HAD TO CHOOSE BETWEEN BREATHING AND LOVING YOU, I WOULD USE MY LAST BREATH TO TELL YOU THAT I LOVE YOU FOREVER.');
  const [signoffText, setSignoffText] = useState('YOURS FOREVER,');
  const [signatureText, setSignatureText] = useState('MATTHEW');

  const [bgColor, setBgColor] = useState('#fcf8f2');
  ...
  const applyPreset = (presetId: string) => {
    ...
    setSalutationText(preset.texts.salutation);
    setBodyText(preset.texts.body);
    setSignoffText(preset.texts.signoff);
    setSignatureText(preset.texts.signature);
    ...
    setShowMargins(preset.styling.showMargins);
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      if (isAdmin) {
        setIsLocked(false);
        if (!token || token === 'demo-token' || token === 'typewriter' || token.length < 10) {
      ...
      try {
        ...
        if (data.design_state) {
          ...
          setSalutationText(ds.salutationText || '');
          setBodyText(ds.bodyText || '');
          setSignoffText(ds.signoffText || '');
          setSignatureText(ds.signatureText || '');
          setBgColor(ds.bgColor || '#fcf8f2');
          setInkColor(ds.inkColor || '#111111');
          setAccentColor(ds.accentColor || '#991b1b');
          setFontFamily(ds.fontFamily || 'Special Elite');
          setShowMargins(ds.showMargins ?? true);
        }
      } catch (err) {
        if (!isAdmin) {
  ...
  const drawTypewriterStructure = () => {
    ...
    const marginL = cw * 0.14; 
    const marginR = cw * 0.86;
    const innerW = cw * 0.72;

    const drawTypewriterMargins = () => {
      if (!showMargins) return;

      const leftMarginLine = new fabric.Line([marginL - 15, 0, marginL - 15, ch], {
        stroke: accentColor,
        strokeWidth: 1.5,
        opacity: 0.6,
        selectable: false,
      ...
      const topCarriageLine = new fabric.Line([0, ch * 0.05, cw, ch * 0.05], {
        stroke: inkColor,
        strokeWidth: 1,
      ...
      canvas.add(topCarriageLine);
    };

    drawTypewriterMargins();

    let y = ch * 0.11;

    if (dateText) {
      const tDate = new fabric.Text(dateText.toUpperCase(), {
        left: marginR,
        top: y,
        originX: 'right',
        fontSize: 11,
    ...
    if (salutationText) {
      const tSalutation = new fabric.Text(salutationText.toUpperCase(), {
        left: marginL,
        top: y,
        fontSize: 12,
        fontFamily: fontFamily,
    ...
    const bodyHeight = tBody.getBoundingRect().height;
    y += bodyHeight + 45;

    if (signoffText) {
      const tSignoff = new fabric.Text(signoffText.toUpperCase(), {
        left: marginR,
        top: y,
        originX: 'right',
        fontSize: 12,
    ...
    if (signatureText) {
      const tSignature = new fabric.Text(signatureText.toUpperCase(), {
        left: marginR,
        top: y,
        originX: 'right',
        fontSize: 14,
  ...
  useEffect(() => {
    ...
    isCheckingToken, tokenError, dateText, salutationText, bodyText, signoffText, 
    signatureText, bgColor, inkColor, accentColor, fontFamily, showMargins
  ]);
    ...
    setTimeout(async () => {
      try {
        ...
        if (format === 'png') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `typewriter-letter.svg`;
          a.click();
          URL.revokeObjectURL(url);
        }

        const designStateJSON = {
          canvasSize, orientation, dateText, salutationText, bodyText, signoffText,
          signatureText, bgColor, inkColor, accentColor, fontFamily, showMargins
        };
    ...
    <div className={`soundwave-poster-page ${isLocked ? 'locked-mode' : ''}`}>
      ...
      <div id="panel" className={isLocked ? 'hidden' : ''}>
        <div className="panel-header">
          <div className="title-group">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M4 7V4h16v3M9 20h6M12 7v13" />
            </svg>
            <h1>Typewriter</h1>
          </div>
        ...
        <div className={`accordion-content${openSections.textFields ? ' open' : ''}`}>
          <div className="form-row">
            <label>Date / Location Header</label>
            <input type="text" value={dateText} onChange={(e) => setDateText(e.target.value)} />
          </div>
          <div className="form-row">
            <label>Salutation (To...)</label>
            <input type="text" value={salutationText} onChange={(e) => setSalutationText(e.target.value)} />
          </div>
          <div className="form-row">
            <label>Mektup / Şiir Body Text</label>
            <textarea 
              value={bodyText} 
        ...
        <button className={`accordion-btn${openSections.styling ? ' open' : ''}`} onClick={() => toggleAccordion('styling')}>
          &#127912; Aged Paper & Ink Styles<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.styling ? ' open' : ''}`}>
          <label className="pf-checkbox-row" style={{ padding: '0 16px' }}>
            ...
            <span style={{ fontSize: '12px', fontWeight: 'bold' }}>Show Typist Margin Guides</span>
          </label>
          <div className="form-row">
          ...
          <div className="form-row">
            <label>Parchment Paper Color</label>
          ...
          <div className="form-row">
            <label>Margin / Signature Highlight</label>
            <div className="color-row">
            ...
            <div className="review-warning-box">
              ...
              <div>
                ...
                <p className="text-red-200/80 text-sm leading-relaxed">
                  Please review your letter carefully. Check typing alignments, spellings, and ink choices.
                </p>
              </div>
            </div>
