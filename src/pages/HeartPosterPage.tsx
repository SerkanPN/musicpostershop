...
const GOOGLE_FONTS = [
  ...
  "Montserrat", "Inter", "Space Mono", "Playfair Display", "Lora", "Prata"
];
...
const PRESETS = [
  {
    id: 'unconditional-vows',
    label: 'Unconditional Vows',
    desc: 'Deep red typographic heart containing your personal wedding vows.',
    texts: {
      title: 'EMILY & MICHAEL',
      date: 'OCTOBER 24, 2026',
      sub: 'OUR WEDDING VOWS',
      body: 'I EMILY, TAKE YOU MICHAEL, TO BE MY WEDDED HUSBAND, TO HAVE AND TO HOLD FROM THIS DAY FORWARD, FOR BETTER, FOR WORSE, FOR RICHER, FOR POORER, IN SICKNESS AND IN HEALTH, TO LOVE AND TO CHERISH, TILL DEATH DO US PART. I PROMISE TO TRUST YOU, HONOR YOU, AND LAUGH WITH YOU IN ALL CIRCUMSTANCES. OUR ADVENTURE HAS ONLY JUST BEGUN. I GAVE YOU MY HEART IN THE VERY FIRST SECOND, AND I PROMISE TO KEEP HOLDING YOUR HAND THROUGH EVERY STORM AND SUNSET.',
  ...
  {
    id: 'first-dance-lyrics',
    label: 'First Dance Lyrics',
    desc: 'Charcoal minimalist heart containing the full lyrics of your wedding song.',
    texts: {
      title: 'SARAH & JOHN',
      date: 'SEPTEMBER 12, 2024',
      sub: 'PERFECT BY ED SHEERAN',
      body: 'I FOUND A LOVE FOR ME. DARLING, JUST DIVE RIGHT IN AND FOLLOW MY LEAD. I FOUND A GIRL, BEAUTIFUL AND SWEET. I NEVER KNEW YOU WERE THE SOMEONE WAITING FOR ME. COZ WE WERE JUST KIDS WHEN WE FELL IN LOVE, NOT KNOWING WHAT IT WAS. I WILL NOT GIVE YOU UP THIS TIME. BUT DARLING, JUST KISS ME SLOW, YOUR HEART IS ALL I OWN, AND IN YOUR EYES, YOU ARE HOLDING MINE. BABY, I AM DANCING IN THE DARK WITH YOU BETWEEN MY ARMS, BAREFOOT ON THE GRASS, LISTENING TO OUR FAVORITE SONG.',
      footer: 'THE FIRST DANCE'
    },
    styling: {
      bg: '#fafafa', 
      heartColor: '#171717', 
  ...
  {
    id: 'mothers-appreciation',
    label: "A Mother's Love",
    desc: "Soft rose palette for thank-you letters and Mother's Day gifts.",
    texts: {
      title: 'DEAREST MOM',
      date: 'MOTHER\'S DAY 2025',
      sub: 'A TRIBUTE TO YOUR LOVE',
      body: 'THANK YOU FOR THE ENDLESS SACRIFICES, THE UNCONDITIONAL LOVE, AND THE WARM HUGS THAT MAKE EVERYTHING BETTER. YOU HAVE BEEN MY ANCHOR IN EVERY STORM AND MY LARGEST INSPIRATION IN LIFE. EVERY SINGLE SUCCESS I COMPLETED BELONGS TO YOUR FAITH IN ME. I PROMISE TO BRING GLORY TO YOUR NAME AND TO HOLD YOUR WISDOM IN EVERY CORNER OF MY HEART. THANK YOU FOR BEING MY HERO AND MY COZIEST COMPASS.',
      footer: 'WE LOVE YOU TO THE MOON & BACK'
    },
    styling: {
      bg: '#fff5f5', 
      heartColor: '#be123c', 
...
export default function HeartPosterPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [activePreset, setActivePreset] = useState<string>('unconditional-vows');
  ...
  const [titleText, setTitleText] = useState('EMILY & MICHAEL');
  const [dateText, setDateText] = useState('OCTOBER 24, 2026');
  const [subText, setSubText] = useState('OUR WEDDING VOWS');
  const [bodyText, setBodyText] = useState('I EMILY, TAKE YOU MICHAEL, TO BE MY WEDDED HUSBAND, TO HAVE AND TO HOLD FROM THIS DAY FORWARD, FOR BETTER, FOR WORSE, FOR RICHER, FOR POORER, IN SICKNESS AND IN HEALTH, TO LOVE AND TO CHERISH, TILL DEATH DO US PART. I PROMISE TO TRUST YOU, HONOR YOU, AND LAUGH WITH YOU IN ALL CIRCUMSTANCES. OUR ADVENTURE HAS ONLY JUST BEGUN. I GAVE YOU MY HEART IN THE VERY FIRST SECOND, AND I PROMISE TO KEEP HOLDING YOUR HAND THROUGH EVERY STORM AND SUNSET.');
  const [footerText, setFooterText] = useState('FOREVER & ALWAYS');

  const [bgColor, setBgColor] = useState('#fcfbf7');
  const [heartColor, setHeartColor] = useState('#991b1b');
  const [textColor, setTextColor] = useState('#1c1917');
  const [fontFamily, setFontFamily] = useState('Playfair Display');

  const showToast = (msg: string) => {
    const el = document.querySelector('.heart-toast') as HTMLElement;
    if (el) {
      el.innerText = msg;
      el.style.opacity = '1';
  ...
  const applyPreset = (presetId: string) => {
    ...
    setHeartColor(preset.styling.heartColor);
    setTextColor(preset.styling.textColor);
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      if (isAdmin) {
        setIsLocked(false);
        if (!token || token === 'demo-token' || token === 'heart' || token.length < 10) {
  ...
  const drawHeartStructure = () => {
    ...
    const drawTypographyHeart = () => {
      const heartW = cw * 0.58;
      const heartH = heartW;

      const heartSVGPath = 'M 12 21.35 l -1.45 -1.32 C 5.4 15.36 2 12.28 2 8.5 C 2 5.42 4.42 3 7.5 3 c 1.74 0 3.41 0.81 4.5 2.09 C 13.09 3.81 14.76 3 16.5 3 C 19.58 3 22 5.42 22 8.5 c 0 3.78 -3.4 6.86 -8.55 11.54 L 12 21.35 z';
      
      const clipPath = new fabric.Path(heartSVGPath, {
        left: cx,
        top: cy,
        width: 24,
        height: 24,
      ...
      const cleanText = bodyText.toUpperCase().replace(/\n/g, ' ').trim();
      const repetitions = Math.max(3, Math.ceil(4000 / (cleanText.length || 1)));
      const repeatingText = Array(repetitions).fill(cleanText).join(' ');

      const textBlock = new fabric.Textbox(repeatingText, {
        left: cx,
        top: cy,
        width: heartW * 1.35, 
        originX: 'center',
    ...
    drawText(titleText, ch * 0.74, 18, '900', textColor, 100);
    drawText(subText, ch * 0.78, 11, '600', textColor, 80);
    drawText(dateText, ch * 0.82, 10, '500', textColor, 60);
  ...
  useEffect(() => {
    ...
    isCheckingToken, tokenError, titleText, dateText, subText, bodyText, 
    footerText, bgColor, heartColor, textColor, fontFamily
  ]);
    ...
    setTimeout(async () => {
      try {
        ...
        if (format === 'png') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `heart-poster.svg`;
          a.click();
          URL.revokeObjectURL(url);
        }

        const designStateJSON = {
          canvasSize, orientation, titleText, dateText, subText, bodyText,
          footerText, bgColor, heartColor, textColor, fontFamily
        };
    ...
    <div className={`soundwave-poster-page ${isLocked ? 'locked-mode' : ''}`}>
      ...
      <div id="panel" className={isLocked ? 'hidden' : ''}>
        ...
        <div className={`accordion-content${openSections.textFields ? ' open' : ''}`}>
          ...
          <div className="form-row">
            <label>Heart Mask Letters / Vows</label>
            <textarea value={bodyText} onChange={(e) => setBodyText(e.target.value)}
              style={{
                width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)',
                borderRadius: '6px', color: 'var(--spotify-text)', padding: '8px 10px', fontSize: '11px',
                fontFamily: 'inherit', minHeight: '130px', resize: 'vertical', outline: 'none'
              }}
            />
          </div>
          <div className="form-row">
        ...
        <div className={`accordion-content${openSections.styling ? ' open' : ''}`}>
          ...
          <div className="form-row">
            <label>Typographic Heart Color</label>
            <div className="color-row">
              <input type="color" value={heartColor} onChange={(e) => setHeartColor(e.target.value)} />
              <input type="text" value={heartColor} onChange={(e) => setHeartColor(e.target.value)} />
            </div>
          </div>
          <div className="form-row">
            ...
            <div className="review-warning-box">
              ...
              <div>
                ...
                <p className="text-red-200/80 text-sm leading-relaxed">
                  Please review your heart typography carefully. Check spelling, structure, and text density.
                </p>
              </div>
            </div>
      ...
      <div className="heart-toast" style={{
        position: 'fixed', bottom: '24px', left: '50%', transform: 'translateX(-50%) translateY(20px)',
