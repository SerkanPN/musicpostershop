...
import { AlertTriangle, Lock, MessageCircle, X, CheckCircle, Upload, Trash2 } from 'lucide-react';
import { supabase } from '../lib/supabase';

const GOOGLE_FONTS = [
  "Fredoka One", "Nunito", "Montserrat", "Inter", "Space Mono",
...
const PRESETS = [
  {
    id: 'sweet-wildflower',
    label: 'Sweet Wildflower',
    desc: 'Soft terracotta and warm peach tones, perfect for nurseries and girls.',
    texts: {
      name: 'LILY ALEXANDER',
      birthDate: 'BORN: MAY 14, 2025',
      stats: 'AGE: 1 YEAR OLD\nTEETH: 8 PEARLY WHITE\nHEIGHT: 76 CM\nWEIGHT: 9.8 KG',
      loveList: '• FRESH SWEET PEACHES\n• PLAYING WITH BUBBLES\n• HER PLUSH BUNNY TOY\n• SPLASHING IN THE TUB\n• GIGGLING WITH DAD',
      canDoList: '• CLAP HER HANDS TO MUSIC\n• STAND UP BY HERSELF\n• SAY "MAMA" AND "DADA"\n• WAVE GOODBYE TO EVERYONE',
      footer: 'OUR GREATEST BLESSING — THE STORY CONTINUES'
    },
    styling: {
      bg: '#fcfaf7', 
      cardBg: '#fbeee6',
  ...
  {
    id: 'little-astronaut',
    label: 'Little Astronaut',
    desc: 'Deep space blues and soft yellow highlights for adventurous little boys.',
    texts: {
      name: 'LEO HARRISON',
      birthDate: 'BORN: MARCH 12, 2025',
      stats: 'AGE: 1 YEAR OLD\nTEETH: 6 SHINING TEETH\nHEIGHT: 78 CM\nWEIGHT: 10.5 KG',
      loveList: '• COLORFUL ROCKET TOYS\n• BANANA MASH PURÉE\n• RUNNING AFTER THE CAT\n• SPLASHING WATER\n• READING PICTURE BOOKS',
      canDoList: '• WALK A FEW STEPS UNAIDED\n• POINT AT SPACE ROCKETS\n• IMITATE ANIMAL SOUNDS\n• THROW SOFT PLUSH BALLS',
      footer: 'EXPLORING THE UNIVERSE — ESTABLISHED 2025'
    },
    styling: {
      bg: '#f0f4f8', 
      cardBg: '#d9e2ec',
  ...
  },
  {
    ...
    desc: 'Earthy sage greens and neutral mountain beige for a clean Scandinavian look.',
    texts: {
      name: 'OLIVER WILDER',
      birthDate: 'BORN: JULY 18, 2025',
      stats: 'AGE: 1 YEAR OLD\nTEETH: 7 TEETH SHINING\nHEIGHT: 75 CM\nWEIGHT: 9.4 KG',
      loveList: '• STACKING WOODEN BLOCKS\n• GREEN APPLE SLICES\n• STROLLER RIDES IN FOREST\n• SPLASHING IN PUDDLES\n• CLAPPING TO DRUM BEATS',
      canDoList: '• CRAWL WITH EXTREME SPEED\n• HOLD HIS OWN BOTTLE\n• DANCE WHEN MUSIC PLAYS\n• RECOGNIZE HIS OWN NAME',
      footer: 'WILD AND FREE — BORN TO EXPLORE'
    },
    styling: {
      bg: '#f4f6f0', 
      cardBg: '#d2d9c4',
...
export default function ToddlerPosterPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [activePreset, setActivePreset] = useState<string>('sweet-wildflower');
  ...
  const [name, setName] = useState('LILY ALEXANDER');
  const [birthDate, setBirthDate] = useState('BORN: MAY 14, 2025');
  const [stats, setStats] = useState('AGE: 1 YEAR OLD\nTEETH: 8 PEARLY WHITE\nHEIGHT: 76 CM\nWEIGHT: 9.8 KG');
  const [loveList, setLoveList] = useState('• FRESH SWEET PEACHES\n• PLAYING WITH BUBBLES\n• HER PLUSH BUNNY TOY\n• SPLASHING IN THE TUB\n• GIGGLING WITH DAD');
  const [canDoList, setCanDoList] = useState('• CLAP HER HANDS TO MUSIC\n• STAND UP BY HERSELF\n• SAY "MAMA" AND "DADA"\n• WAVE GOODBYE TO EVERYONE');
  const [footer, setFooter] = useState('OUR GREATEST BLESSING — THE STORY CONTINUES');

  const [imgUrl, setImgUrl] = useState<string | null>(null);
  const [bgColor, setBgColor] = useState('#fcfaf7');
  const [cardBgColor, setCardBgColor] = useState('#fbeee6');
  const [inkColor, setInkColor] = useState('#4a2711');
  const [accentColor, setAccentColor] = useState('#d35400');
  const [fontFamily, setFontFamily] = useState('Fredoka One');

  const showToast = (msg: string) => {
    const el = document.querySelector('.tod-toast') as HTMLElement;
    if (el) {
      el.innerText = msg;
      el.style.opacity = '1';
  ...
  const applyPreset = (presetId: string) => {
    ...
    setName(preset.texts.name);
    setBirthDate(preset.texts.birthDate);
    setStats(preset.texts.stats);
    setLoveList(preset.texts.loveList);
    setCanDoList(preset.texts.canDoList);
    setFooter(preset.texts.footer);

    setBgColor(preset.styling.bg);
    setCardBgColor(preset.styling.cardBg);
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      if (isAdmin) {
        setIsLocked(false);
        if (!token || token === 'demo-token' || token === 'toddler' || token.length < 10) {
      ...
      try {
        ...
        if (data.design_state) {
          ...
          setName(ds.name || '');
          setBirthDate(ds.birthDate || '');
          setStats(ds.stats || '');
          setLoveList(ds.loveList || '');
          setCanDoList(ds.canDoList || '');
          setFooter(ds.footer || '');
          setImgUrl(ds.imgUrl || null);
          setBgColor(ds.bgColor || '#fcfaf7');
          setCardBgColor(ds.cardBgColor || '#fbeee6');
          setInkColor(ds.inkColor || '#4a2711');
          setAccentColor(ds.accentColor || '#d35400');
          setFontFamily(ds.fontFamily || 'Fredoka One');
        }
      } catch (err) {
        if (!isAdmin) {
  ...
  const drawToddlerStructure = async () => {
    ...
    const drawText = (txt: string, size: number, weight: string, align: 'left' | 'center' | 'right', customY = y, customColor = inkColor) => {
    ...
    drawText(name, 22, '900', 'center');
    y += 30;

    drawText(birthDate, 10, '600', 'center', y, accentColor);
    y += 24;
    ...
    const colW = innerW * 0.31;
    const cardH = ch * 0.22;

    const drawCard = (xPos: number, header: string, bodyText: string, hasImage = false) => {
      ...
      const tHeader = new fabric.Text(header.toUpperCase(), {
        left: xPos + 12,
        top: y + 12,
        fontSize: colW * 0.09,
        fontFamily: fontFamily,
      ...
      if (!hasImage) {
        const tBody = new fabric.Textbox(bodyText.toUpperCase(), {
          left: xPos + 12,
          top: y + 35,
          width: colW - 24,
          fontSize: colW * 0.075,
    ...
    drawCard(marginL, 'MY STATS', stats);

    const centerCardX = marginL + colW + innerW * 0.035;
    drawCard(centerCardX, 'ME IN 2026', '', true);

    if (imgUrl) {
      try {
        const imgObj = await fabric.Image.fromURL(imgUrl, { crossOrigin: 'anonymous' });
        const scale = Math.max(colW / imgObj.width!, cardH / imgObj.height!);
        imgObj.set({
          left: centerCardX,
          top: y,
          scaleX: scale,
    ...
    const rightCardX = centerCardX + colW + innerW * 0.035;
    drawCard(rightCardX, 'THINGS I LOVE', loveList);

    y += cardH + 20;

    const bottomCardH = ch * 0.18;
    const bottomCard = new fabric.Rect({
      left: marginL,
      top: y,
      width: innerW,
      height: bottomCardH,
    ...
    canvas.add(bottomCard);

    const tBottomHeader = new fabric.Text('I CAN DO THESE THINGS...', {
      left: marginL + 16,
      top: y + 14,
      fontSize: 12,
      fontFamily: fontFamily,
    ...
    canvas.add(tBottomHeader);

    const tBottomBody = new fabric.Textbox(canDoList.toUpperCase(), {
      left: marginL + 16,
      top: y + 36,
      width: innerW - 32,
      fontSize: 10.5,
    ...
    canvas.add(tBottomBody);

    y += bottomCardH + 24;

    const bottomLine = new fabric.Line([marginL, y, marginR, y], {
      stroke: inkColor,
      strokeWidth: 2,
      selectable: false
    });
    ...
    drawText(footer, 10, '700', 'center', y);
  ...
  useEffect(() => {
    ...
    isCheckingToken, tokenError, name, birthDate, stats, loveList, canDoList, 
    footer, imgUrl, bgColor, cardBgColor, inkColor, accentColor, fontFamily
  ]);
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setImgUrl(url);
      showToast('Photo placed successfully inside the milestone layout.');
    };
    ...
    setTimeout(async () => {
      try {
        ...
        if (format === 'png') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `toddler-milestone-poster.svg`;
          a.click();
          URL.revokeObjectURL(url);
        }

        const designStateJSON = {
          canvasSize, orientation, name, birthDate, stats, loveList, canDoList,
          footer, imgUrl, bgColor, cardBgColor, inkColor, accentColor, fontFamily
        };
  ...
  return (
    <div className={`soundwave-poster-page ${isLocked ? 'locked-mode' : ''}`}>
      ...
      <div id="panel" className={isLocked ? 'hidden' : ''}>
        ...
        <button className={`accordion-btn${openSections.photo ? ' open' : ''}`} onClick={() => toggleAccordion('photo')}>
          &#128247; Milestones Photo<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.photo ? ' open' : ''}`}>
          <div className="form-row">
            <label>Upload center grid photo</label>
            <div className="flex gap-2 items-center mt-2">
              <button className="upload-compact-btn" onClick={() => document.getElementById('tod-photo-upload')?.click()}>
                <Upload className="w-3.5 h-3.5" /> Upload Photo
              </button>
              <input type="file" id="tod-photo-upload" accept="image/*" onChange={handlePhotoUpload} style={{ display: 'none' }} />
              {imgUrl && (
                <button className="trash-btn" onClick={removePhoto} title="Remove Photo">
        ...
        <button className={`accordion-btn${openSections.textFields ? ' open' : ''}`} onClick={() => toggleAccordion('textFields')}>
          &#128294; Infographic Contents<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.textFields ? ' open' : ''}`}>
          <div className="form-row">
            <label>Child's Big Name</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="form-row">
            <label>Birth Date String</label>
            <input type="text" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} />
          </div>
          <div className="form-row">
            <label>Stats Box Content</label>
            <textarea value={stats} onChange={(e) => setStats(e.target.value)}
              style={{
                width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)',
                borderRadius: '6px', color: 'var(--spotify-text)', padding: '8px 10px', fontSize: '11px',
                fontFamily: 'inherit', minHeight: '80px', resize: 'vertical', outline: 'none'
              }}
            />
          </div>
          <div className="form-row">
            <label>Things I Love List</label>
            <textarea value={loveList} onChange={(e) => setLoveList(e.target.value)}
              style={{
                width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)',
                borderRadius: '6px', color: 'var(--spotify-text)', padding: '8px 10px', fontSize: '11px',
                fontFamily: 'inherit', minHeight: '80px', resize: 'vertical', outline: 'none'
          ...
          <div className="form-row">
            <label>I Can Do List (Bottom)</label>
            <textarea value={canDoList} onChange={(e) => setCanDoList(e.target.value)}
              style={{
                width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)',
                borderRadius: '6px', color: 'var(--spotify-text)', padding: '8px 10px', fontSize: '11px',
                fontFamily: 'inherit', minHeight: '80px', resize: 'vertical', outline: 'none'
        ...
        <button className={`accordion-btn${openSections.styling ? ' open' : ''}`} onClick={() => toggleAccordion('styling')}>
          &#127912; Playful Styles & Colors<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.styling ? ' open' : ''}`}>
          <div className="form-row">
            <label>Nursery Font</label>
          ...
          <div className="form-row">
            <label>Grid Card Background</label>
            <div className="color-row">
            ...
            <div className="review-warning-box">
              ...
              <div>
                ...
                <p className="text-red-200/80 text-sm leading-relaxed">
                  Please review your child milestone panel carefully. Check stats, rules, and photos.
                </p>
              </div>
            </div>
