...
const GOOGLE_FONTS = [
  "Playfair Display", "Times New Roman", "Prata", "Vollkorn", "Lora", 
  "Crimson Text", "Cinzel", "Inter", "Montserrat", "Oswald"
];
...
const PRESETS = [
  {
    ...
    desc: 'Vintage celebration front page for 18th, 30th, or 50th birthdays.',
    texts: {
      paperName: 'THE DAILY CHRONICLE',
      dateStr: 'OCTOBER 24, 1994',
      priceStr: 'PRICE 10 CENTS',
      headline: 'A LEGEND IS BORN IN THE CITY!',
      col1: 'SPECIAL REPORT: OUTSTANDING SOURCES CONFIRM THAT A FUTURE ICON HAS OFFICIALLY ENTERED THE WORLD STAGE TODAY. WITNESSES DESCRIBE THE NEWBORN AS RADIANT, ALREADY DEMONSTRATING HIGH SYSTEMS INTELLIGENCE.',
      col2: 'LOCAL CELEBRATIONS HAVE BROKEN OUT ACROSS THE ENTIRE COUNTRY. SCIENTISTS PREDICT A SPECTACULAR JOURNEY AHEAD FOR THIS EXTRAORDINARY INDIVIDUAL. READ THE FULL BIOGRAPHY INSIDE.'
    },
    styling: {
      bg: '#fcf8ee', 
      ink: '#111111', 
  ...
  },
  {
    ...
    desc: 'Romantic front page celebrating the greatest love story of the century.',
    texts: {
      paperName: 'THE ROMANCE TIMES',
      dateStr: 'AUGUST 24, 2025',
      priceStr: 'PRICE: ONE KISS',
      headline: 'THE GREATEST LOVE STORY OF OUR CENTURY!',
      col1: 'EXCLUSIVE COVERAGE: THE INCREDIBLE ADVENTURE OF THIS SPECTACULAR COUPLE REACHES ANOTHER STAGE. FRIENDS AND FAMILY CELEBRATE THE DAY THEY FIRST MET AND EMBARKED ON A FOREVER JOURNEY.',
      col2: 'EXPERTS AGREE THAT THEIR UNCOMPROMISING DEVOTION, ENDLESS LAUGHTER, AND MUTUAL RESPECT OUTLAST ALL TRADITIONAL METRICS. "A MATCH MADE IN THE HEAVENS," SAYS THE LOCAL PRESS.'
    },
    styling: {
      bg: '#fbfbfb', 
      ink: '#1c1917', 
...
export default function NewspaperPosterPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [activePreset, setActivePreset] = useState<string>('birthday-legend');
  ...
  const [paperName, setPaperName] = useState('THE DAILY CHRONICLE');
  const [dateStr, setDateStr] = useState('OCTOBER 24, 1994');
  const [priceStr, setPriceStr] = useState('PRICE 10 CENTS');
  const [headline, setHeadline] = useState('A LEGEND IS BORN IN THE CITY!');
  const [col1, setCol1] = useState('SPECIAL REPORT: OUTSTANDING SOURCES CONFIRM THAT A FUTURE ICON HAS OFFICIALLY ENTERED THE WORLD STAGE TODAY. WITNESSES DESCRIBE THE NEWBORN AS RADIANT, ALREADY DEMONSTRATING HIGH SYSTEMS INTELLIGENCE.');
  const [col2, setCol2] = useState('LOCAL CELEBRATIONS HAVE BROKEN OUT ACROSS THE ENTIRE COUNTRY. SCIENTISTS PREDICT A SPECTACULAR JOURNEY AHEAD FOR THIS EXTRAORDINARY INDIVIDUAL. READ THE FULL BIOGRAPHY INSIDE.');

  const [imgUrl, setImgUrl] = useState<string | null>(null);
  const [bgColor, setBgColor] = useState('#fcf8ee');
  const [inkColor, setInkColor] = useState('#111111');
  const [fontFamily, setFontFamily] = useState('Playfair Display');

  const showToast = (msg: string) => {
    const el = document.querySelector('.news-toast') as HTMLElement;
    if (el) {
      el.innerText = msg;
      el.style.opacity = '1';
  ...
  const applyPreset = (presetId: string) => {
    ...
    setPaperName(preset.texts.paperName);
    setDateStr(preset.texts.dateStr);
    setPriceStr(preset.texts.priceStr);
    setHeadline(preset.texts.headline);
    setCol1(preset.texts.col1);
    setCol2(preset.texts.col2);
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      if (isAdmin) {
        setIsLocked(false);
        if (!token || token === 'demo-token' || token === 'newspaper' || token.length < 10) {
      ...
      try {
        ...
        if (data.design_state) {
          ...
          setPaperName(ds.paperName || '');
          setDateStr(ds.dateStr || '');
          setPriceStr(ds.priceStr || '');
          setHeadline(ds.headline || '');
          setCol1(ds.col1 || '');
          setCol2(ds.col2 || '');
          setImgUrl(ds.imgUrl || null);
          setBgColor(ds.bgColor || '#fcf8ee');
          setInkColor(ds.inkColor || '#111111');
  ...
  const drawNewspaperStructure = async () => {
    ...
    drawText(paperName, 26, '900', 'center');
    y += 35;

    drawLine(4, 18);

    drawText(dateStr, 11, '700', 'left', y - 4);
    drawText(priceStr, 11, '700', 'right', y - 4);

    drawLine(2, 24);

    const tHeadline = new fabric.Textbox(headline.toUpperCase(), {
      left: cw / 2,
      top: y,
      width: innerW,
      originX: 'center',
    ...
    canvas.add(tHeadline);

    const headlineH = tHeadline.getBoundingRect().height;
    y += headlineH + 18;

    const picW = innerW;
    const picH = ch * 0.32;

    const picBox = new fabric.Rect({
      left: marginL,
      top: y,
      width: picW,
      height: picH,
    ...
    if (imgUrl) {
      try {
        ...
        const scale = Math.max(picW / imgObj.width!, picH / imgObj.height!);
        imgObj.set({
          left: marginL,
          top: y,
          scaleX: scale,
    ...
    const colW = innerW * 0.47;

    const tCol1 = new fabric.Textbox(col1.toUpperCase(), {
      left: marginL,
      top: y,
      width: colW,
      fontSize: 10,
    ...
    const tCol2 = new fabric.Textbox(col2.toUpperCase(), {
      left: marginR,
      top: y,
      width: colW,
      originX: 'right',
  ...
  useEffect(() => {
    ...
    isCheckingToken, tokenError, paperName, dateStr, priceStr, headline, 
    col1, col2, imgUrl, bgColor, inkColor, fontFamily
  ]);
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setImgUrl(url);
      showToast('Photo applied to vintage press layout.');
    };
    ...
    setTimeout(async () => {
      try {
        ...
        if (format === 'png') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `newspaper-poster.svg`;
          a.click();
          URL.revokeObjectURL(url);
        }

        const designStateJSON = {
          canvasSize, orientation, paperName, dateStr, priceStr, headline,
          col1, col2, imgUrl, bgColor, inkColor, fontFamily
        };
    ...
    <div className={`soundwave-poster-page ${isLocked ? 'locked-mode' : ''}`}>
      ...
      <div id="panel" className={isLocked ? 'hidden' : ''}>
        ...
        <button className={`accordion-btn${openSections.photo ? ' open' : ''}`} onClick={() => toggleAccordion('photo')}>
          &#128247; Press Photo<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.photo ? ' open' : ''}`}>
          <div className="form-row">
            <label>Upload front-page image</label>
            <div className="flex gap-2 items-center mt-2">
              <button className="upload-compact-btn" onClick={() => document.getElementById('news-photo-upload')?.click()}>
                <Upload className="w-3.5 h-3.5" /> Upload Photo
              </button>
              <input type="file" id="news-photo-upload" accept="image/*" onChange={handlePhotoUpload} style={{ display: 'none' }} />
              {imgUrl && (
        ...
        <button className={`accordion-btn${openSections.textFields ? ' open' : ''}`} onClick={() => toggleAccordion('textFields')}>
          &#128294; Newspaper Columns<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.textFields ? ' open' : ''}`}>
          ...
          <div className="form-row">
            <label>Article Column 1 (Left)</label>
            <textarea value={col1} onChange={(e) => setCol1(e.target.value)}
              style={{
                width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)',
          ...
          <div className="form-row">
            <label>Article Column 2 (Right)</label>
            <textarea value={col2} onChange={(e) => setCol2(e.target.value)}
              style={{
                width: '100%', background: 'var(--input-bg)', border: '1px solid var(--input-border)',
                borderRadius: '6px', color: 'var(--spotify-text)', padding: '8px 10px', fontSize: '11px',
                fontFamily: 'inherit', minHeight: '80px', resize: 'vertical', outline: 'none'
        ...
        <button className={`accordion-btn${openSections.styling ? ' open' : ''}`} onClick={() => toggleAccordion('styling')}>
          &#127912; Vintage Ink Styles<span className="arrow">&#9660;</span>
        </button>
        <div className={`accordion-content${openSections.styling ? ' open' : ''}`}>
          ...
          <div className="form-row">
            <label>Aged Newsprint Color</label>
            ...
            <div className="review-warning-box">
              ...
              <div>
                ...
                <p className="text-red-200/80 text-sm leading-relaxed">
                  Please review your newspaper front page carefully. Check column formats, spelling, and contrast.
                </p>
              </div>
            </div>
      ...
      <div className="news-toast" style={{
        position: 'fixed', bottom: '24px', left: '50%', transform: 'translateX(-50%) translateY(20px)',
